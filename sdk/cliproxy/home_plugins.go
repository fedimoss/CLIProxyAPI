...
func (s *Service) stageHomePluginTasksWithClient(ctx context.Context, cfg *config.Config, client *home.Client) ([]homePluginTaskWork, error) {
	...
	if client == nil {
		// No Home client available (e.g. overlay applied before the home
		// subscriber starts): skip plugin task staging without failing the
		// overlay so the config merge still takes effect.
		return nil, nil
	}
	if ctx == nil {
		ctx = context.Background()
