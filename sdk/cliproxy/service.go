...
type Service struct {
	...
	// appliedHealthProbeInterval tracks the interval currently driving the local
	// OAuth health probe auto-refresh loop so config reloads can restart it on change.
	appliedHealthProbeInterval time.Duration
}

// oauthHealthProbeInterval resolves the configured OAuth health probe interval.
