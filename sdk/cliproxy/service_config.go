...
func (s *Service) applyManagerConfig(ctx context.Context, commit configCommit) bool {
	...
	if !commit.cfg.Home.Enabled {
		nextHealthProbeInterval := s.oauthHealthProbeInterval(commit.cfg)
		if s.appliedHealthProbeInterval != nextHealthProbeInterval {
			...
			s.appliedHealthProbeInterval = nextHealthProbeInterval
		}
	}
	return true
}
